...
A3. Workshop Studio에 접속해서 좌측 사이드바 "Get AWS CLI Credentials"를 선택합니다. 이후 Linux or macOS (bash) 탭에서 명령어 4줄을 복사하고 Git Bash 등의 터미널 환경에 입력합니다. 
...
신규 사용자에 적용할 **이메일(Email), 사용자 이름(Given Name) / 성(Family Name) 정보**를 입력합니다.
```sh
./kiroctl init
```
...
A5. 아래와 같은 상태에 갇혀 있을 경우, Game 서버 컨테이너에 문제가 발생했을 가능성이 있습니다.
