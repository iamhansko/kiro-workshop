...
> [!NOTE]
>
> 인터넷 브라우저에서 새로고침한 이후에도 변경사항이 적용되지 않았다면 `./scripts/init.sh` 셸 스크립트를 실행하여 Spirit of Kiro 게임을 재시작합니다.
> ```sh copy
> # Windows
> ./scripts/init.sh
> 
...
모든 Spec 문서( requirements.md / design.md / tasks.md )는 `.kiro/specs/` 디렉토리에 저장됩니다. 이 파일들은 프로젝트의 중요한 설계 문서이므로, 소스코드와 함께 코드 저장소(Git)에 커밋할 것을 권장합니다.
